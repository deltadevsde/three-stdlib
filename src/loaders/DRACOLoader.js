...
class DRACOLoader extends Loader {
  ...
  setWorkerSourceURL(workerSourceURL) {
    this.workerSourceURL = workerSourceURL;
  ...
  _getWorker(taskID, taskCost) {
    return this._initDecoder().then(() => {
      if (this.workerPool.length < this.workerLimit) {
        const worker = new Worker(URL.createObjectURL(new Blob([this.workerSourceURL])));
        worker._callbacks = {};
        worker._taskCosts = {};
        worker._taskLoad = 0;
        worker.postMessage({
